...
data Term : List Ty -> List Ty -> Ty -> Type where
  ...
  Shut   : Term d (Box a::d) a -> Term d g (Box a)
  Letbox : Term d g (Box a) -> Term (a::d) g b -> Term d g b

axiomK : Term d g (Box (a ~> b) ~> Box a ~> Box b)
axiomK = Lam $ Lam $ Letbox (Var $ There Here)
                            (Letbox (Var Here)
                                    (Shut $ App (Var $ There $ There Here) (Var $ There Here)))

axiom4 : Term d g (Box a ~> Box (Box a))
axiom4 = Lam $ Letbox (Var Here) (Shut $ Shut $ Var $ There Here)

axiomW : Term d g (Box (Box a ~> a) ~> Box a)
axiomW = Lam $ Letbox (Var Here) (Shut $ App (Var $ There Here) (Var Here))
